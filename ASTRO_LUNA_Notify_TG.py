    ...
    for attempt in range(retries):
        try:
            ...
            usdc_raw = next((b['amount'] for b in balances
                             if b['denom']== 'ibc/B559A80D62249C8AA07A380E2A2BEA6E5CA9A6F079C912C3A9E9B494105E4F81'), None)
            astro_raw = next((b['amount'] for b in balances
                              if b['denom'].startswith('factory/') and '/astro' in b['denom']), None)
...
# Function to fetch balance from the Terra blockchain with retries, delay, and back-off
def fetch_uluna_balance(address, retries=3, delay=5):
    ...
    for attempt in range(1, retries+1):
        try:
            ...
            return int(raw) / 10**6 if raw else 0.0

        except requests.exceptions.RequestException as e:
            print(f"Error fetching LUNA (attempt {attempt}): {e}")
            if attempt < retries:
                print(f"Retrying in {delay}s...")
                time.sleep(delay)
            else:
...
def compare_balances(neutron_addresses, terra_address, interval=3):
    ...
    while True:
        ...
        # Debug prints
        print(f"Checking LUNA balance for Terra ({terra_address})...")
        print(f"Previous LUNA balance: {prev_luna}")
        print(f"Current LUNA balance:  {curr_luna}")
        ...
        for label, addr in neutron_addresses:
            ...
            # Debug prints for USDC
            print(f"Checking USDC balance for {label}...")
            print(f"Previous USDC balance: {prev_usdc}")
            print(f"Current USDC balance:  {curr_usdc}")
            if curr_usdc != prev_usdc:
                msg = generate_message(label, "USDC", curr_usdc, curr_usdc - prev_usdc)
                print(msg)
                send_telegram_message(TELEGRAM_BOT_TOKEN, TELEGRAM_CHAT_ID, msg)

            # Debug prints for ASTRO
            print(f"Checking ASTRO balance for {label}...")
            print(f"Previous ASTRO balance: {prev_astro}")
            print(f"Current ASTRO balance:  {curr_astro}")
...
if __name__ == "__main__":
    neutron_addresses = [
        ("BOT_ASTRO_LUNA", "neutron1vzvagczrx7xz28g8wvqwettdaeyhgyn8a99774"),
